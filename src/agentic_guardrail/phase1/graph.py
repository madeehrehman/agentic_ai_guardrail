...
def compile_graph(
    *,
    ...
    """
    checkpointer: explicit saver (tests use MemorySaver).
    checkpoint_backend: ``memory`` | ``sqlite`` | ``auto`` when checkpointer is None.
    db_path: SQLite file path when using sqlite backend.
    """
